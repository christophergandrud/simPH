...
\paragraph{Penalized Splines} Another useful and more flexible way of modeling nonlinearity \citep[see][]{Keele2008} are penalized splines.
...
\cite{Brambor2006} argue that multiplicative interactions are more easily communicated as marginal effects. For Cox PH models the marginal effect ($ME_{x}$) of $x$ on the hazard rate for different values of $z_{i}$ is given by:
%
\begin{equation}
